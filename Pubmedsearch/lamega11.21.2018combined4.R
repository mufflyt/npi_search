...
devtools::install_github("r-lib/memoise", force=TRUE)
devtools::install_github("HenrikBengtsson/future", force=TRUE)
library(memoise)
library(future)
...
DATA_DIR <- path("~/Dropbox/Pubmedsearch/Scraper/CU")
WORKING_DIR <- path("~/Dropbox/Pubmedsearch/Scraper/CU") 
# escape sequences for spaces in path
SCRIPT_DIR <- path(WORKING_DIR, "europepmc")
GOBA_FILE <- "GOBA_divorce_study.xlsx"
DOX_FILE <- path(DATA_DIR, "DoximityData-shortened.csv")
...
goba %<>%
  ...
  mutate(query = str_c(
    "(", query,
    ") AND (KW:femal* OR KW:obstetr* OR KW:gynecol*) AND (AFF:obstetr* OR AFF:gynecolog* OR AFF:family OR AFF:maternal OR AFF:Female OR AFF:Pelvic OR AFF:Reproduct*) AND (LANG:eng OR LANG:en OR LANG:us) AND (SRC:MED)"    
  )) %>% 
...
flog.info("Writing combined data to %s", path(DATA_DIR, "combined4.rds.bz"))
write_rds(combined, path(DATA_DIR, "combined4.rds.bz"), compress = "bz")
...
## (GM) write to csv
flog.info("Writing combined data to %s", path(DATA_DIR, "combined_unnested4.csv"))
combined %>% 
  filter(map_lgl(search, not_null)) %>%
  unnest(search) %>%
  write_csv(path(DATA_DIR, "combined_unnested4.csv"))

#Store data from search strategy 4
library(tidyverse) 
combined4 <- combined
remove(combined)
citations4 <- citations
remove(citations)
doctor_names4 <- doctor_names
remove(doctor_names)
g_index4 <- g_index
remove(g_index)
h_index4 <- h_index
remove(h_index)
m_quotient4 <- m_quotient
remove(m_quotient)
colnames(combined4)
